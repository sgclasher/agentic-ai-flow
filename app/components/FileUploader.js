...
export default function FileUploader() {
  ...
  const { loadAgenticData, setAgenticData, setServiceNowUrl, error } = useAgenticStore();
  const [url, setUrl] = useState('');

  const handleFileChange = async (e) => {
    const file = e.target.files[0];
    if (file) {
      // Save URL first, then load file data
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="url-input-group">
            <input
              ...
              className="servicenow-url-input"
            />
            <button type="submit" className="url-submit-button">Save URL</button>
          </div>
        </form>
      </div>
